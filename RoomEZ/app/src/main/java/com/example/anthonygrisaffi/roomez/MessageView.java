...
public class MessageView {
    ...
    private Message.RecipientStatus getMessageStatus(Message msg) {
        ...
        if (!msg.getSentByUserId().equalsIgnoreCase(GroupMessage.getUserID()))
        ...
        for (int i = 0; i < GroupMessage.getAllParticipants().size(); i++) {

            //Don't check the status of the current user
            String participant = GroupMessage.getAllParticipants().get(i);
            if (participant.equalsIgnoreCase(GroupMessage.getUserID()))
                continue;
